...
async execute(client, message, args) {
 ...
 await sentMessage.react('745639913889726614')
 ...
 { max: 1, time: 30000 }).then(collected => {
  if (collected.first().emoji.id == '745638221920075816') {
    const help = new Discord.MessageEmbed()
    help.setTitle('Moderation commands')
    help.setDescription("`ban`,`kick`")
    sentMessage.edit(help)
  }
  else { 
    if (collected.first().emoji.id == '745638932187971766') {
      const help = new Discord.MessageEmbed()
      help.setTitle("IDK")
      sentMessage.edit(help)
    }
